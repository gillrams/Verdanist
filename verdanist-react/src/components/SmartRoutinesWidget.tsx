      ...
      <div className="flex flex-col gap-3">
        {routines.map((routine, i) => (
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className="rounded-2xl p-4 bg-card/60 backdrop-blur-md flex items-center justify-between border border-border shadow-sm cursor-pointer hover:bg-card/90 transition-colors"
          >
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-500 shrink-0 border border-purple-500/15">
                <span className="material-symbols-rounded text-[20px]">schedule</span>
              </div>
              <div>
                <p className="text-foreground font-bold text-sm">{routine.label}</p>
                <p className="text-muted-foreground text-xs flex items-center gap-1 mt-0.5 font-medium">
