...
export default function AIAssistantWidget({ temperature = 29, humidity = 70 }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full bg-white/70 dark:bg-[#0A2F1F]/80 backdrop-blur-xl rounded-[23px] p-5 border border-white/50 dark:border-white/10 relative z-10">
        <div className="flex items-start gap-4">
          <div className={`w-12 h-12 rounded-2xl bg-white dark:bg-black/30 flex items-center justify-center shadow-sm border border-black/5 dark:border-white/5 shrink-0 ${colorClass}`}>
          ...
          <div>
            ...
            <p className="text-[#0A2F1F] dark:text-white/90 text-sm font-bold mb-1 leading-snug">
              {suggestion}
            </p>
            <p className="text-[#0A2F1F]/70 dark:text-white/50 text-xs font-semibold flex items-center gap-1">
