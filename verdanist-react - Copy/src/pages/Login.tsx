...
export default function Login() {
  ...
  const handleOAuthLogin = async (provider: 'google') => {
    ...
    try {
      ...
      // Supabase will handle the redirect to Google accounts page automatically.
      // We do not need to navigate manually here.
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-[100dvh] w-full flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-[#E8F8EE] via-[#F2FAF4] to-[#E0F4E8]">
      ...
      <motion.div
        ...
        className="absolute top-[5%] left-[10%] w-[400px] h-[400px] bg-green-300/30 rounded-full blur-[100px] z-0 pointer-events-none"
      />
      <motion.div
        animate={{ y: [0, 40, 0], scale: [1, 1.2, 1] }}
        transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        className="absolute bottom-[5%] right-[10%] w-[500px] h-[500px] bg-teal-200/40 rounded-full blur-[120px] z-0 pointer-events-none"
      />
      ...
      >
        <div className="bg-white/80 backdrop-blur-2xl border border-white shadow-[0_20px_60px_rgba(10,47,31,0.08)] rounded-[36px] p-8 lg:p-12 relative overflow-hidden">
          ...
          <div className="text-center mb-10 mt-4">
            ...
            />
            <h1 className="text-[32px] font-extrabold text-[#0A2F1F] tracking-tight mb-2">Welcome Back</h1>
            <p className="text-[#0A2F1F]/60 font-medium">Log in to manage your smart greenhouse.</p>
          </div>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-5">
            ...
            <div className="flex items-center justify-between px-1 pt-2">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input type="checkbox" className="rounded border-gray-300 text-green-500 focus:ring-green-500/50 w-4 h-4 transition-all" />
                <span className="text-sm font-bold text-gray-500 group-hover:text-[#0A2F1F] transition-colors">Remember me</span>
              </label>
              <a href="#" className="text-sm font-bold text-green-600 hover:text-green-500 transition-colors">Forgot password?</a>
            </div>

            <button
              type="submit"
              disabled={loadingProvider !== null}
              className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white font-extrabold py-4 rounded-2xl mt-6 shadow-[0_10px_25px_rgba(16,185,129,0.3)] hover:shadow-[0_15px_35px_rgba(16,185,129,0.4)] hover:-translate-y-1 active:scale-[0.98] transition-all flex justify-center items-center h-[58px] text-lg border border-green-400/50"
            >
          ...
          <p className="mt-10 text-center text-sm text-gray-500 font-semibold">
            ...
            <Link to="/register" className="font-extrabold text-green-600 hover:text-green-500 transition-colors">Sign up here</Link>
          </p>
        </div>
      </motion.div>
    </div>
